...
class RFOF_Interactive_Interface {
    ...
    init() {
        // Event-Listener für Buttons und Enter-Taste
        ...
        // Hinzufügen des Willkommenstexts bei Initialisierung
    ...
    addMessage(text, sender, isCode = false) {
        ...
        if (isCode) {
            // Highlights Code in der Chat-Historie
    ...
    handleSendMessage() {
        ...
        // Simuliert eine AI-Antwort basierend auf der Nachricht (ohne API-Call)
        setTimeout(() => {
    ...
    handleExecuteCode() {
        ...
        if (code.toLowerCase().includes('python') || code.includes('print(')) {
            ...
            try {
                // Ein einfaches eval, um JS-Code auszuführen. Achtung: Nicht für Produktion verwenden.
                result = eval(code);
                if (result === undefined) {
        ...
        // Fügt das Ergebnis direkt in das Output-Fenster ein
    ...
    handleCorrection() {
        ...
        // Simuliert eine Korrektur, z.B. das Hinzufügen eines Kommentars
        ...
        // Fügt korrigierten Code und Erklärung in die Chat-Historie ein
